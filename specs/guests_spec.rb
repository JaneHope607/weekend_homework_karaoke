...
class GuestsTest < MiniTest::Test
    ...
    def test_favourite_song_present__false
        favourite_song = @guest2.favourite_song_present(@room1)
        assert_equal("Aw no that sucks!", favourite_song)
    end

    def test_favourite_song_present__true
        @room1.add_songs(@song1.track_name)
        favourite_song = @guest1.favourite_song_present(@room1)
        assert_equal("Woo this is my favourite song!", favourite_song)
    end
